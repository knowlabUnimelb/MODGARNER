...
whichCondition = 'Sat'; % {'Bri', 'Sat'}
whichtask = 1:4; % 1-4

%% Test data
cols = {'sub', 'task', 'prev', 'item', 'acc', 'rt'};
...
data = dlmread(fullfile(dataloc, sprintf('test_sep_seq%sAllRT.dat', whichCondition)));
...
for i = 1:3
    for j = 1:3
        clear d1 d2
                   % Task i,      % Near Item,  % Previous item j
            d1 = d( d(:,2) == i & d(:,3) == 3 & d(:,4) == testItems(j,1), [1 5]);
        if j == 1
                   % Task i,      % Near Item,  % Previous item not a rep
            d2 = d( d(:,2) == i & d(:,3) == 3 & d(:,4) ~= testItems(j,1), [1 5]);
        else
                    % Task i,      % Near Item,  % Previous item not the effect or the rep
...
d1 = d( d(:,2) == 3 & d(:,3) == 3 & d(:,4) == 3, [1 5]);
d2 = d( d(:,2) == 4 & d(:,3) == 3 & d(:,4) == 3, [1 5]);

d3 = d( d(:,2) == 3 & d(:,3) == 3 & d(:,4) == 1, [1 5]);
d4 = d( d(:,2) == 4 & d(:,3) == 3 & d(:,4) == 1, [1 5]);

d5 = d( d(:,2) == 3 & d(:,3) == 3 & d(:,4) == 4, [1 5]);
d6 = d( d(:,2) == 4 & d(:,3) == 3 & d(:,4) == 4, [1 5]);
...
if strcmp(whichCondition, 'Bri')
    dlmwrite('mldata_sep_rt_bri_acc.dat', [1*ones(size(mldata, 1), 1), mldata], '\t')
    dlmwrite('mldata_sep_rt_idv_bri_acc.dat', [1*ones(size(mldata2, 1), 1), mldata2], '\t')
else
    dlmwrite('mldata_sep_rt_sat_acc.dat', [2*ones(size(mldata, 1), 1), mldata], '\t')
    dlmwrite('mldata_sep_rt_idv_sat_acc.dat', [2*ones(size(mldata2, 1), 1), mldata2], '\t')
end
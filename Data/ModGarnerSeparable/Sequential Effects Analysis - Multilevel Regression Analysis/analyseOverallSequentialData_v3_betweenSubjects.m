...
data =  dlmread(fullfile(dataloc, sprintf('test_sep_seq%sCorrectRT.dat', whichCondition)));
...
if strcmp(whichCondition, 'Bri')
    dlmwrite('mldata_sep_rt_bri.dat', [1*ones(size(mldata, 1), 1), mldata], '\t')
    dlmwrite('mldata_sep_rt_idv_bri.dat', [1*ones(size(mldata2, 1), 1), mldata2], '\t')
else
    dlmwrite('mldata_sep_rt_sat.dat', [2*ones(size(mldata, 1), 1), mldata], '\t')
    dlmwrite('mldata_sep_rt_idv_sat.dat', [2*ones(size(mldata2, 1), 1), mldata2], '\t')
end
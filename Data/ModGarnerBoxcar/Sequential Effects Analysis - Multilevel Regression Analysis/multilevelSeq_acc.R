...
datafile <- "mldata_boxcar_rt_lin_acc.dat" # ML data are repetitions vs all items except repetitions
lindataall <- read.table(datafile, sep = "\t", header = FALSE, row.names = NULL, col.names = c("con", "sub", "tsk", "itm", "rep", "acc"))

# Summarize data as counts
lindata <- aggregate(acc~sub+tsk+itm+rep, data=lindataall, FUN=sum, na.rm=T)
counts <-  aggregate(acc~sub+tsk+itm+rep, data=lindataall, FUN=length)
lindata$N <- counts$acc
lindata$err <- lindata$N - lindata$acc

datafile <- "mldata_boxcar_rt_idv_lin_acc.dat" # ML data are repetitions vs all items except repetitions
lindataallFI <- read.table(datafile, sep = "\t", header = FALSE, row.names = NULL, col.names = c("con", "sub", "tsk", "itm", "acc"))

# Summarize data as counts
lindataFI <- aggregate(acc~sub+tsk+itm, data=lindataallFI, FUN=sum, na.rm=T)
counts <-  aggregate(acc~sub+tsk+itm, data=lindataallFI, FUN=length)
lindataFI$N <- counts$acc
lindataFI$err <- lindataFI$N - lindataFI$acc

datafile <- "mldata_boxcar_rt_sat_acc.dat" # ML data are repetitions vs all items except repetitions
...
datafile <- "mldata_boxcar_rt_idv_sat_acc.dat" # ML data are repetitions vs all items except repetitions
...
linControlRepetitionEffect <- brm(formula = err | trials(N) ~ rep + (1|sub), 
           data = bcn, 
...
linControlPushingEffect <- brm(formula = err | trials(N) ~ rep + (1|sub), 
               data = bcf, 
               family = binomial("logit"),
               prior = c(set_prior("normal(0, 1)", class = "b")), 
               warmup = 1000, iter = 2000, chains = 4, sample_prior = TRUE, 
...
linControlPullingEffect <- brm(formula = err | trials(N) ~ rep + (1|sub), 
                               data = bca, 
                               family = binomial("logit"),
                               prior = c(set_prior("normal(0, 1)", class = "b")), 
                               warmup = 1000, iter = 2000, chains = 4, sample_prior = TRUE, 
...
#########################################################################################################################################################
#########################################################################################################################################################
# linghtness, Correlated
# linCorrelatedRepetitionEffect <- lmer(rt ~ rep + (1|sub), bln)
bln <- data[data$tsk == 2 & data$itm == 1, ]
linCorrelatedRepetitionEffect <- brm(formula = err | trials(N) ~ rep + (1|sub), 
                                  data = bln, 
                                  family = binomial("logit"),
                                  prior = c(set_prior("normal(0, 1)", class = "b")), 
                                  warmup = 1000, iter = 2000, chains = 4, sample_prior = TRUE, 
...
hypothesis(linCorrelatedRepetitionEffect, "rep < 0")
#########################################################################################################################################################
fdt_adapt_delta_level = 0.99995
blf <- data[data$tsk == 2 & data$itm == 2, ]
# linCorrelatedPushingEffect <- lmer(rt ~ rep + (1|sub), bcf)
linCorrelatedPushingEffect <- brm(formula = err | trials(N) ~ rep + (1|sub), 
                               data = blf, 
                               family = binomial("logit"),
                               prior = c(set_prior("normal(0, 1)", class = "b")), 
                               warmup = 1000, iter = 2000, chains = 4, sample_prior = TRUE, 
...
linCorrelatedPullingEffect <- brm(formula = err | trials(N) ~ rep + (1|sub), 
                               data = bla, 
                               family = binomial("logit"),
                               prior = c(set_prior("normal(0, 1)", class = "b")), 
                               warmup = 1000, iter = 2000, chains = 4, sample_prior = TRUE, 
...
linFilteringRepetitionEffect <- brm(formula = err | trials(N) ~ rep + (1|sub), 
                                     data = bfn, 
                                    family = binomial("logit"),
                                     prior = c(set_prior("normal(0, 1)", class = "b")), 
                                     warmup = 1000, iter = 2000, chains = 4, sample_prior = TRUE, 
...
hypothesis(linFilteringRepetitionEffect, "rep < 0")
#########################################################################################################################################################
fdt_adapt_delta_level = 0.999995
bff <- data[data$tsk == 3 & data$itm == 2, ]
# linFilteringPushingEffect <- lmer(rt ~ rep + (1|sub), bcf)
linFilteringPushingEffect <- brm(formula = err | trials(N) ~ rep + (1|sub), 
                                  data = bff, 
                                 family = binomial("logit"),
                                  prior = c(set_prior("normal(0, 1)", class = "b")), 
                                  warmup = 1000, iter = 2000, chains = 4, sample_prior = TRUE, 
...
linFilteringPullingEffect <- brm(formula = err|trials(N) ~ rep + (1|sub), 
                                  data = bfa, 
                                 family = binomial("logit"),
                                  prior = c(set_prior("normal(0, 1)", class = "b")), 
                                  warmup = 1000, iter = 2000, chains = 4, sample_prior = TRUE, 
...
dataidv = lindataFI
fdt_adapt_delta_level = 0.999999999995
# Rep vs Rep
idvN <- dataidv[dataidv$itm == 1, ]
idvN$tsk <- idvN$tsk - 3
linIrrelevantDimensionRep <- brm(formula = err|trials(N) ~ tsk + (1|sub), 
                                 data = idvN, 
                                 family = binomial("logit"),
                                 prior = c(set_prior("normal(0, .75)", class = "b")), 
                                 warmup = 1000, iter = 2000, chains = 4, sample_prior = TRUE, 
                                 control = list(adapt_delta = fdt_adapt_delta_level))
...
linIrrelevantDimensionPush <- brm(formula = err|trials(N) ~ tsk + (1|sub), 
                                 data = idvF, 
                                 family = binomial("logit"),
                                 prior = c(set_prior("normal(0, 1)", class = "b")), 
                                 warmup = 1000, iter = 2000, chains = 4, sample_prior = TRUE, 
...
linIrrelevantDimensionPull <- brm(formula = err|trials(N) ~ tsk + (1|sub), 
                                  data = idvA, 
                                  family = binomial("logit"),
                                  prior = c(set_prior("normal(0, 1)", class = "b")), 
                                  warmup = 1000, iter = 2000, chains = 4, sample_prior = TRUE, 
...
#########################################################################################################################################################
allObjects = ls()
save(allObjects, file = "boxcarACC.Rdata")
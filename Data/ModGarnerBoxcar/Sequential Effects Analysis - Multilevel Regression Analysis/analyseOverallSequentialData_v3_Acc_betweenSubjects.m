...
data = dlmread(fullfile(dataloc, sprintf('test_boxcar_seq%sAllRT.dat', whichCondition)));
...
if strcmp(whichCondition, 'Lin')
    dlmwrite('mldata_boxcar_rt_lin_acc.dat', [1*ones(size(mldata, 1), 1), mldata], '\t')
    dlmwrite('mldata_boxcar_rt_idv_lin_acc.dat', [1*ones(size(mldata2, 1), 1), mldata2], '\t')
else
    dlmwrite('mldata_boxcar_rt_sat_acc.dat', [2*ones(size(mldata, 1), 1), mldata], '\t')
    dlmwrite('mldata_boxcar_rt_idv_sat_acc.dat', [2*ones(size(mldata2, 1), 1), mldata2], '\t')
end
...
subject = 205; % Odd is line/even is saturation 101:104, 202:205
condition = 2;
sessions = 2:6; 
lastsession = 6;
...
for i = 1:numel(sessions)
    %BOXCAR
    data = [data; dlmread(fullfile(dataloc, sprintf('GARNER_SEPARABLE_BOXCAR_%03d_%02d_s%02d.dat', subject, condition, sessions(i))))];
end
data(data(:,strcmp(cols, 'task')) == 6, strcmp(cols, 'task')) = 5; 

%% Setup output label
if ismember(subject, [101, 102, 103, 104])
    outfileidx = sprintf('L%d', find(subject == [101, 102, 103, 104]));
elseif ismember(subject, [201, 202, 203, 204 205])
    outfileidx = sprintf('S%d', find(subject == [205, 202, 203, 204])); %205 instead of 201 as 201 excluded
else 
...
fdata = [];
sesstaskorder = data(data(:,strcmp(cols, 'trl')) == 1,3:4); %for integral, change to 3:4; for separable, change to 4:5
for i = 2:lastsession
...
fid = fopen(sprintf('Garner_BOXCAR_sub%s_ordered_tagged_msec.dat', outfileidx), 'wt');

...
switch debug
    ...
    case false
        ...
        nExpTrials = 120; % Number of experimental trials = 120 * nBlocks
...
outputfile = sprintf('GARNER_SEPARABLE_%03d_%02d.dat', subject, session);
...
%         Priority(0);
    ...
    if bblocks == nBlocks
        ...
        if bonusscore > .90
            fins{6} = sprintf('Bonus = Yes, Mean Correct in Last %d trials = %4.2f', bonusNtrials,bonusscore);
        else
            fins{6} = sprintf('Bonus = No, Mean Correct in Last %d trials = %4.2f', bonusNtrials, bonusscore);
        end
        disp(fins{6})
    end
end

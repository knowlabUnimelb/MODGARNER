function plotPosteriorPredictive_makeplot2(mm, sm, pm, ma, sa, pa, varargin)
% Reorganize plot for posteriors
% Create separate figures for control, correlated, and filtering RT and
% p(Error).  Each figure is 1 x 3 subplots; each supblot contains different
...
%% p(Error) plots
plotCondition(ma, sa, pa, 1, 'Control', ylimsacc, 'p(Error)');
plotCondition(ma, sa, pa, 2, 'Correlated', ylimsacc, 'p(Error)')
plotFilteringCondition(ma, sa, pa, 3, 'Filtering', ylimsacc,'p(Error)');
...
for i = 1:numel(items)
    ...
    errorbar(1:6, iset(:,3), sset(:,3)./sqrt(4), elines{i}, 'LineWidth', 1, 'MarkerSize', 10);
    hold on
...
for i = 1:numel(items)
    ...
    for j = 1:2
        ...
        if j == 1
            ...
            errorbar((1:6), iset(:,3), sset(:,3)./sqrt(4), elines{j}{i}, 'LineWidth', 1, 'MarkerSize', 10, 'MarkerFaceColor', 'w');

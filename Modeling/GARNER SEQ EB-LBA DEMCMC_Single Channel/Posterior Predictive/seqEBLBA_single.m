function varargout = seqEBLBA_single(parms, stimloc, data, cols, varargin)
...
% w2 = 1 - w1;
% w = [w1; w2];
c = (parms([1 1 1 1 2])); % Specificity
r = repmat(R_METRIC, 1, 5); % r-Metric
b1 = parms(3:7);
b2 = 1 - b1;
br = [b1; b2];
wp = parms(8);                  % Probability of using previous stimulus
bh = parms(9);
shep = SHEPARD_EXPONENT;

% p = stmgcm(exemplars, item, task, w, c, r, br, wp, bh, igp);
p = stmgcm_single(exemplars, item, task, c, r, br, wp, bh, igp, shep);
...
%%
function p = stmgcm_single(exemplars, item, task, c, r, b, wp, bhigher, igp, shep)
...
%          (w(2,i) * (abs(repmat(y,1,numel(y)) - repmat(y', numel(y), 1)).^r(i)))).^(1/r(i));
    d = abs(repmat(y,1,numel(y)) - repmat(y', numel(y), 1));

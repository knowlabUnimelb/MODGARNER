...
c_mu_mu = 1; c_mu_sigma = 1;   % Fixed hyper prior values
...
cf_mu_mu = 1; cf_mu_sigma = 1;     % Fixed hyper prior values
...
wp_a = 5; % wp_a ~ Normal(1, _5)T(0_5, inf)
wp_b = 1; % wp_b ~ Normal(1, _5)T(0_5, inf)

wp_a_mu    = 5; wp_a_sigma = .5;
wp_b_mu    = 1; wp_b_sigma = .5;

%%
bh = 1.3;  % Change bias parameter - bhigher(beta)

% bh ~ Normal(mu, sigma)T(0, inf)
bh_mu = 1.3;     % wp_mu ~ Normal(1, _5)T(0, inf)
bh_sigma = .5; % wp_sigma ~ Gamma(1, 1)

bh_mu_mu = 1.5; bh_mu_sigma = 1;
